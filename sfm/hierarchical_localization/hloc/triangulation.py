...
def trajectory_alignment(
    ...
    init_dir = Path(os.path.join(cfg["local_path"], cfg[f"{cam}_sfm_init_subdir"]))
    opti_dir = Path(os.path.join(cfg["local_path"], cfg[f"{cam}_sfm_opti_subdir"]))
    out_dir = Path(os.path.join(cfg["local_path"], cfg[f"{cam}_sfm_output_subdir"]))
    ...
    database = opti_dir / "database.db"
    reference = pycolmap.Reconstruction(init_dir)
    ...
    if not skip_geometric_verification:
        if estimate_two_view_geometries:
            estimation_and_geometric_verification(database, pairs_txt, verbose)
        else:
            geometric_verification(image_ids, reference, database, feature_path, pairs_txt, match_path)

    out_dir.mkdir(parents=True, exist_ok=True)

    subprocess.run([colmap_exe, "trajectory_alignment", "--config_path", config_path, "--cam_name", cam])
    subprocess.run([colmap_exe, "model_converter", "--input_path", str(opti_dir), "--output_path", str(out_dir), "--output_type", "TXT"])
    subprocess.run([colmap_exe, "model_analyzer", "--path", str(out_dir)])
